...
import { Input, Output, Task, Fn, Work } from "../work_api";
...
const mapped2 = map(1 as unknown as Work<Task<number, 2>>, 1 as unknown as <T extends Fn<number, number>>(val: T) => T);
const mapped3 = map(1 as unknown as Task<number, 2>, 1 as unknown as <T extends Fn<number, number>>(val: T) => T);
